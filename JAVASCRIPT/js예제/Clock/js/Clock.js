...
$(function(){
	...
	let script = {
		...
		curTime: function(){
			...
			$('.clock-wrap p.time-write').html((hours < 10 ? `0${hours}` : hours) + ":" + (minutes < 10 ? `0${minutes}` : minutes) + ":" + (seconds < 10 ? `0${seconds}` : seconds));
	...
	let analogClock = {
		...
		liveFunc: () => {
			...
			const liveH = liveTime.getHours();
			const liveM = liveTime.getMinutes();
			const liveS = liveTime.getSeconds();
			
			handH.style.transform = `rotate(${(liveH * 60 + liveM) * 0.5}deg)`;
			handM.style.transform = `rotate(${liveM * 6}deg)`;
			handS.style.transform = `rotate(${liveS * 6}deg)`;
	...
	setInterval(analogClock.liveFunc, 1000);
